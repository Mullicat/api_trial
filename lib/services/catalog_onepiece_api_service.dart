// lib/services/catalog_onepiece_api_service.dart
import 'dart:convert';
...
class OnePieceTcgService {
  ...
  Future<List<TCGCard>> getCards({
    ...
    try {
      ...
      if (filters.isNotEmpty) {
        ...
        developer.log('Applied filters for One Piece: $filters');
      } else {
        queryParams['name'] = '';
        developer.log('No filters applied for One Piece, fetching all cards');
      }
      ...
      developer.log('API Request for One Piece: $uri');
      developer.log(
      ...
      if (response.statusCode == 200) {
        ...
        return cardsJson.map((card) {
          ...
          developer.log(
            'Parsing One Piece card: name=${cardJson['name']}, id=${cardJson['id']}',
          );
      ...
      developer.log('Error fetching One Piece cards: $e');
  ...
  Future<TCGCard?> getCard({required String id}) async {
    try {
      ...
      developer.log('API Request for One Piece getCard: $uri');
      developer.log(
      ...
      if (response.statusCode == 200) {
        ...
        developer.log(
          'Parsing One Piece card: name=${cardJson['name']}, id=${cardJson['id']}, rarity=${cardJson['rarity']}',
        );
        return _parseCard(cardJson);
      } else if (response.statusCode == 500 &&
          response.body.contains('Datos no encontrados')) {
        developer.log('One Piece card not found for ID: $id');
        return null;
      } else {
        String errorMessage =
